      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        <div className="space-y-2.5">
          <h3 className="font-bold text-xl flex items-center gap-1.5">
            <span className="hanko !w-5 !h-5 !text-sm !rounded">診</span>5つの相性軸
          </h3>
          {c.axes.map((ax, i) => (
            <div key={ax.label}>
              <div className="flex items-center gap-3">
                <span className="w-28 text-sm font-bold shrink-0">
                  {ax.icon} {ax.label}
                </span>
                ...
                <span className="text-gold text-sm w-16 text-right tracking-tight shrink-0">
                  {"★".repeat(ax.n)}
                  {"☆".repeat(5 - ax.n)}
                </span>
              </div>
              <p className="text-sm text-sub mt-0.5 ml-[7.75rem] leading-relaxed max-sm:ml-0">{ax.text}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-indigo/5 border border-indigo/15 rounded-xl p-4 space-y-1">
          ...
          <p className="text-sm text-sub leading-relaxed">{c.gogyo.text}</p>
        </div>
        ...
        {c.clusterRel && (
          <div className="bg-gold/10 border border-gold/30 rounded-xl p-4 space-y-1">
            ...
            <p className="text-sm text-sub leading-relaxed">{c.clusterRel}</p>
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 gap-2 text-sm">
          <div className="bg-paper rounded-xl px-3.5 py-2.5">
            <span className="font-bold text-primary-text">✦ {a.label}から贈る方言</span>
            <p className="mt-1 font-bold text-sm">{c.luckyA}</p>
          </div>
          <div className="bg-paper rounded-xl px-3.5 py-2.5">
            <span className="font-bold text-primary-text">✦ {b.label}から贈る方言</span>
        ...
        <div className="space-y-2 text-sm">
          <div className="bg-paper rounded-xl px-3.5 py-2.5">
            ...
            <p className="mt-1.5">
              仲直りの合言葉は {b.type.dialect}で「<b className="text-primary-text">{nakanaori}</b>」
            </p>
          </div>
          <div className="bg-paper rounded-xl px-3.5 py-2.5">
