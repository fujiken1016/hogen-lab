  ...
  return (
    <article className="max-w-2xl mx-auto space-y-6 text-sm leading-relaxed">
      ...
      <section className="space-y-2">
        ...
        <p>
          一部のページに設置している「この変換、どうでしたか？」欄で送信された内容（評価と、任意でご記入いただく200文字までの本文）は、
          サービス改善の目的でのみ、当サイトのサーバー（Cloudflare Workers KV）に保存します。
          お名前・メールアドレス・IPアドレスは保存しません。返信もできません。
          個人情報はご記入にならないでください（送信時に、メールアドレス・電話番号・URLと判定した部分は自動的に削除しています）。
        </p>
      </section>
      ...
      <section className="space-y-2">
        ...
        <p>
          また、「読みもの」記事には楽天アフィリエイト等による広告リンクを掲載しており、該当箇所には「PR」と明示しています。
          リンク経由で商品が購入された場合、当サイトが売上の一部を報酬として受け取ることがありますが、購入者の支払額が変わることはありません。
          商品の販売・発送・サポートは各販売事業者が行います。
        </p>
      </section>
