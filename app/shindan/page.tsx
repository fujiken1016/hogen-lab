...
// 35方言をただランダムに出すと自分の方言が6問中ほぼ登場せず（期待値0.7回）、
// 全員が標準語（クールバランサー）に吸われるため、出身地方の言い方を必ず選択肢に入れる。
// 出題数は固定（6＋4、2026-09-21に8+6→6+4へ短縮。理由=GA4ファネル実測でshindan_start→shindan_complete間の離脱率27.03%が
// 全ステップ中最大。質問プール自体は変えず出題数だけ絞る＝プール（言葉12・性格12）から診断開始時にランダムに選ぶ
const PHRASE_Q_COUNT = 6;
const PERSONA_Q_COUNT = 4;
...
/**
 * 診断ページ下部の静的解説。intro画面と結果画面の両方で同じものを出すので、
 * 2箇所に同じ文面を書かずにここへ集約する（2026-09-14）。
 * 数値は lib/data.ts の実データを数えて書いている。書き換えるときは数え直すこと。
 */
function ShindanIntro() {
  return (
    <ToolIntro
      heading="方言タイプ診断について"
      paragraphs={[
        "この診断は、ふだんの言い回しと性格の好みを全10問たずねて、あなたの言葉がどの方言圏のタイプに近いかを出す無料ツールです。出身地は聞きません。手がかりにするのは「気を抜いたときに出る言い方」だけです。",
        "だから、引っ越しが多かった人や、標準語で育ったつもりの人ほど意外な結果になります。結果は全35タイプのご当地キャラクターのどれかで、性格の傾向・恋愛傾向・ほかのタイプとの相性まで出ます。登録もインストールも要りません。所要時間は約2分です。",
      ]}
      steps={[
        {
          t: "育った地域を選ぶ",
          d: "「育った地域・ゆかりのある地域は？」で8地方から1つ選びます。選んだ地方の言い方が優先して出題されますが、結果を縛るものではありません。",
        },
        {
          t: "言葉づかいを6問答える",
          d: "「『捨てておいて』を自然に言うと？」のように、同じ意味の言い方が方言別に並びます。近いものを選んでください。どれも違うときは、自由入力で自分の言い方を書けます。",
        },
        {
          t: "性格を4問答える",
          d: "「飲み会で、いつの間にかやっている役回りは？」など。選んだあと「どれくらい当てはまるか」を1〜5で答えます。この数字が結果の「強さレベル」になります。",
        },
        {
          t: "結果を見る",
          d: "上位の方言が成分の百分率で並び、いちばん強いものがあなたのタイプになります。結果画面のコードを友だちに渡すと、ふたりの相性も出せます。",
        },
      ]}
      facts={[
        {
          h: "何を判定しているのか",
          body: [
            "判定しているのは出身地ではなく、言葉の好みです。材料は2つだけで、6問で選んだ方言フレーズと、4問の性格の答えです。",
            "出題に使うフレーズは、このサイトが持っている36の言い方（標準語を含む）ぶん・432件の方言フレーズから選ばれます。言葉づかいの質問は12種、性格の質問も12種あり、診断のたびにそこから6問と4問がランダムに選び直されます。",
            "地方を選ぶと、最初は選択肢の一部にその地方の言い方が優先で入ります。地元の言い方を選ばない回答が2回続くとその枠は1つに減り、4回続くと全国の言い方だけになります。しっくりこないまま地元に寄せ続けない、という作りです。",
          ],
        },
        {
          h: "配点の考え方",
          body: [
            "標準語を選んだときは0点です。誰でも話せる言い方は、どの方言圏かの証拠になりません。",
            "方言フレーズは「珍しさ」で配点が変わります。ほかの方言と言い方が被るほど弱い証拠なので1.4点、その方言でしか言わない形なら2.2点、という幅を持たせています。",
            "似た言い方の方言にも部分点が波及します。たとえば「ほかしといて」で関西弁を選んでも、京都弁・神戸弁に少しだけ点が入ります。方言は県境でぷつりと切れないからです。",
            "性格の質問は選んだ方向に1点、次点の候補に0.5点・0.25点・0.1点と薄く配ります。最後に回答の組み合わせから決まる小さな値を足し、小数1桁で表示します。1問違えば成分の比率が全部変わるので、判で押したような結果になりません。",
          ],
        },
      ]}
      faq={[
        {
          q: "出身地と違う結果が出ました。外れていますか？",
          a: "外れというより、この診断が出身地を当てるものではないからです。見ているのは「いま自分がしっくりくる言い方」で、引っ越し・家族の出身・よく見る動画の影響がそのまま出ます。",
        },
        {
          q: "標準語しか使いません。診断できますか？",
          a: "できます。標準語の選択肢は0点なので、4問の性格の答えと、「あえて選ぶならこっち」と思った言い方だけで判定します。標準語が続くと出題は全国の言い方に広がります。",
        },
        {
          q: "何度やっても同じ結果になりますか？",
          a: "なりません。毎回12種の中から6問と4問が選び直されるので、出題そのものが変わります。答えが少し違えば成分の比率も変わります。",
        },
        {
          q: "答えた内容はどこに保存されますか？",
          a: "あなたの端末のブラウザ内だけです。自由入力で書いた言い方も端末に残り、「みんなの方言辞書」のページで投稿フォームにそのまま呼び出せます。",
        },
        {
          q: "結果の言い方が自分の地元と違います。直せますか？",
          a: "診断の中の自由入力と、「みんなの方言辞書」から自分の言い方を書き残せます。辞典に載っていない言い回しを見つけたら、そちらに書いておいてください。",
        },
      ]}
      related={[
        { href: "/aishou", label: "💞 友だちとの相性診断" },
        { href: "/dict", label: "📖 みんなの方言辞書" },
...
function ShindanPage() {
  ...
  // 今回の診断で使う質問セット（プールから6＋4問をランダム抽出。初期値は先頭固定でSSRと一致させる）
      ...
      <div className="max-w-3xl mx-auto text-center space-y-5 seigaiha rounded-3xl border border-line px-6 py-8 relative overflow-hidden">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sub">
            言葉づかい6問＋性格4問でわかる、あなたの「方言キャラタイプ」。
            <br />
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-sub">全10問・245通り以上・登録不要・無料</p>
        </div>
