...
export default function DevCharsPage() {
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold">キャラデザイン確認室</h1>

      <section className="space-y-3">
        <h2 className="text-lg font-bold">シークレット2体（実寸より大きめ・全アニメ有効）</h2>
        <div className="grid sm:grid-cols-2 gap-4">
          {SECRETS.map((s) => (
            ...
            >
              ...
              <p className="text-white/50 text-xs">{s.rateLabel}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-bold">全35キャラ（目・口・ほっぺのバリエーション確認用）</h2>
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {TYPES.filter((t) => t.slug !== "std").map((t) => (
            <div key={t.slug} className="card !rounded-xl p-2 text-center">
              ...
              <div className="text-[10px] font-bold text-primary truncate">{MASCOT_NAMES[t.slug]}</div>
            </div>
          ))}
        </div>
      </section>
